...
当前版本状态: 已修复一次真实使用中发现的问题（write-through + 共享规范
  修改权限，见 adr/0002），2026-07-24 起纳入 git 版本控制
...
（2026-07-22）针对 hermes 在真实使用中暴露的"记录被当成收尾动作"问题，
两轮迭代修复：先加 write-through 原则和共享规范确认流程，再经六家 AI
聚焦咨询后升级为"未记录即未完成"+骨架先行机制，重新打包为新版
`blackbox.skill` 交付；同步补记 CHANGELOG 和 ADR-0002。

...
- Default branch: `vnext` (a mutable development/current branch; static docs do not assert a live tip SHA/tree here). The default-branch switch from `main` to `vnext` has already been performed; no future switch gate remains. `vnext` is allowed to advance beyond the immutable v2.0.0 release snapshot.
...
- v2.0.0 is released: `pyproject.toml` version is `2.0.0`; the annotated Git tag `v2.0.0` exists and peels to `f26db56d0cabdfae900a3befc222547a23c3d909` (tree `69b582239920c71f7d0588a38d6b0d1658842abf`); the GitHub Release `v2.0.0` is published (<https://github.com/LeiD215/blackbox/releases/tag/v2.0.0>); the package is **not** published to PyPI. BCL 1.0 root `LICENSE` is in the released tree.
- Formal release status: the v2.0.0 release snapshot (tag/Release, see above) is anchored on the `vnext` default branch at `f26db56d0cabdfae900a3befc222547a23c3d909`; the live `vnext` branch is a mutable successor that is allowed to advance beyond that snapshot without changing it. The package is not published to PyPI. Qualified legal review of BCL 1.0 is deferred and non-blocking per USER decision; any future default-branch/license-finalization decision remains a separate USER gate.
- Next decision boundary (separate USER gates, NOT performed by this commit):
  - qualified legal review of BCL 1.0 (deferred and non-blocking).
  The v2.0.0 release/tag decision has been performed; the default-branch switch from `main` to `vnext` is already complete and is not a remaining gate.
